...
This project is operated by a set of specialized subagents defined in `.claude/agents/` (gitignored): `task-manager` (coordinates and tracks only, never implements — note it has no ability to invoke other agents itself, despite its description), `web-developer` (templates/content models/Parsley — never deploys/publishes), `jenny` (independent, read-only verification that a web-developer diff actually matches the original spec and this file's conventions — runs after web-developer; checks source only, never renders the site, never fixes anything itself), then the user reviews the diff themselves (a fixed gate — jenny's pass doesn't substitute for it), then `qa-tester` (tests only a feature branch's own live preview — goes straight to the browser and reports what it finds, rather than pre-checking GitHub Actions/zesty-deploy.yml run history to decide whether to test; does not test dev rollup or stage — see Deployment model above), then `karen` (runs as a fixed step immediately after qa-tester reports a pass, not on-demand — reality-checks that result by reading web-developer's actual changed file(s) and directly viewing the feature branch's live preview and the stage website in a real browser, rather than trusting qa-tester's summary; runs no scripts — no `deploy:dry`, no `git`/`gh` — and doesn't check dev rollup or production; hands off to `jenny` for a full spec/CLAUDE.md audit or `qa-tester` for a full feature-branch test pass when the question needs one). `api-integrator` (maintains the MCP server, read-only tools only), `documentation-maker` (writes `/docs`, marks new/changed docs "Draft — pending review" until the user signs off), and `release-notes` (customer-facing changelog entries) round out the roster.
