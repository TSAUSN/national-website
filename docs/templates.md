...
Corrected 2026-09-20 (same day, follow-up): the entry below originally documented the CODA-2533 fix as addressing a "hung connection" via a client-side fetch timeout. That framing was a **hypothesis, not the confirmed cause**. The actual root cause has since been confirmed — an unbounded `find_in_set` scan in `webengine/views/events` hitting WebEngine's 3000-query cap — and is corrected in place below. The client-side fetch-timeout fix is still real and still in place, but as defense-in-depth for genuine network hangs, not as the fix for what was actually breaking the page in production.
...
- **Bug (CODA-2533):** the Events Landing Page got stuck on an infinite loading spinner. First reported on USA Southern Territory's page (`/usa-southern-territory/events/`), but confirmed to affect every territory/division/location Events Landing Page, since `upcoming-events` is one shared module/template, not something Southern-specific.
- **Confirmed root cause (corrected 2026-09-20 — supersedes the earlier "hung fetch" hypothesis below):** the actual cause lives in `webengine/views/events` (the `/events.json` endpoint this module calls), not in this module's JS. See that view's entry in the Ajax-JSON table below for the full explanation — in short, an unbounded `find_in_set` scan over the entire `events` content model was hitting WebEngine's hard 3000-query-execution cap, and that error is suppressed on production/live renders, so the front-end here never received valid JSON and just spun forever instead of showing an error.
- **Original hypothesis (kept for history, not the actual cause):** it was first assumed that `fetchEvents()`'s plain `fetch('/events.json...')` (no timeout/`AbortController`) could simply *hang* — never resolve or reject — and that this explained the infinite spinner. That assumption was reasonable given the symptom but has been superseded now that the query-cap error has been confirmed live. (The `locationZUID`/`locationModel` cookie logic in `client-global-navigation` was also checked and ruled out — it's uniform across all territories.)
- **Fix #1 — defense-in-depth, still in place, unchanged by the root-cause correction:** `fetchEvents()` wraps its `fetch()` call in an `AbortController` with a 30s timeout (`EVENTS_FETCH_TIMEOUT_MS`), clears the timeout in a `finally`, and treats a non-OK HTTP response (`!model.ok`) as a logged failure returning `false` rather than trying to parse a bad response. `initEvents()` guards on the result: if `fetchEvents()` didn't return an array (timeout, abort, bad status, or thrown error), it shows a "We're unable to load events right now. Please try again later." message and returns, instead of letting the subsequent `.sort()` throw on `false`. This remains valid for genuine network hangs — it's just not what was actually breaking the page in production.
- **Fix #2 — the actual fix for the production bug:** `webengine/views/events` now bounds the query with `LIMIT 0, 500 SORT BY event.event_date_with_time DESC` in all three model branches. See the Ajax-JSON table entry below for detail and residual risks.
- **Net effect:** the Events Landing Page should no longer hang on production; a genuine network-level hang is still bounded to ~30s by Fix #1. Neither fix is QA'd/merged/deployed yet.
...
| `events.json` (view name `events`, no file extension) | `zuid`, `model` | Events for a Territory (`model=6-deab97cfd9-wb5km4`), Division (`6-acb19a94bd-4q8ftj`), or Location/property (`6-b4c9aba69c-h2nqvm`), each branch filtering with `{{each events as event WHERE find_in_set('{$zuid}', <territories\|divisions\|property>) LIMIT 0, 500 SORT BY event.event_date_with_time DESC}}`. **Confirmed root cause of CODA-2533 (corrected 2026-09-20 — this is the real fix, not the fetch-timeout in `modules/upcoming-events`):** before the fix, all three branches had no `LIMIT`/`SORT` at all — an unbounded `find_in_set` scan over the *entire* `events` content model, scanned before the per-row filter is applied, so the cost scales with the model's total row size, not with how many events are tagged to the requesting territory/division/location. This unbounded scan was confirmed live to hit WebEngine's hard **3000 query-execution cap** (reproduced via `/events.json?zuid=7-b4aeafb8dd-033tjr&model=6-deab97cfd9-wb5km4`, USA Southern Territory, on the branch preview — returned a "WebEngine Query Error — Maximum query limit of 3000 reached" page). That error page is a **stage/preview-only diagnostic and is suppressed in production/live renders** — so in production the front-end simply received no valid JSON, which is why the Events Landing Page hung on an infinite spinner rather than showing any error. Not Southern-specific — every territory/division/location shares the same risk as the underlying `events` model grows. **Fix:** added `LIMIT 0, 500 SORT BY event.event_date_with_time DESC` to all three branches; sorting newest-first means truncation (if it ever recurs) drops oldest events first rather than an arbitrary slice. **Residual risks (open, not resolved by this fix):** (1) it's assumed but not independently verified against a live API call that WebEngine's query engine short-circuits the scan once `LIMIT` rows are matched rather than scanning the full model and truncating output afterward — worth an `api-integrator` check if the query-cap error recurs after this ships; (2) `LIMIT 0, 500` is a judgment call sized to leave headroom under the 3000 cap for the per-row `cities.filter()`/`states.filter()` lookups this template also does per matched row — any territory/division/location with legitimately >500 tagged events would still silently lose events beyond that cap, far less likely than before but not impossible; (3) recurring events that only populate `date_time_summary` (no `event_date_with_time`) have no comparable field for the new `SORT BY` and could sort toward the bottom, risking truncation first in a high-volume territory — not fixable without a schema/content change. The client-side `fetch()` timeout in `modules/upcoming-events` (see that module's entry above) is unchanged and remains in place as separate, complementary defense-in-depth for genuine network hangs — it was not superseded by this fix, just wasn't what actually broke the page in production. See `custom-patterns.md` §14 for the broader pattern (the same unbounded `find_in_set`-in-`WHERE` recurs in ~30+ other views repo-wide, though this is the only one confirmed to have hit the query cap so far). |
...
10. **CODA-2533** is Draft — pending review, implemented on branch `coda-2533` only, not yet QA'd/merged/deployed. Do not cite either fix as shipped in customer-facing release notes until a human confirms the merge. Two fixes, corrected 2026-09-20: the **confirmed root cause and actual fix** is the `LIMIT 0, 500 SORT BY event.event_date_with_time DESC` added to `webengine/views/events` (all three model branches) — see the `events.json` row in the Ajax-JSON table above for full detail and residual risks. The client-side `AbortController`/timeout fix in `modules/upcoming-events` (see that module's entry above) is a separate, still-valid defense-in-depth fix for genuine network hangs, but was **not** what was actually breaking the page in production — that earlier "hung fetch" framing was a hypothesis, since superseded.
