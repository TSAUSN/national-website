...
## 14. `find_in_set()` scan over a whole content model in a `{{each ... WHERE ...}}` — confirmed root cause of CODA-2533, fixed in `webengine/views/events`

**Corrected/upgraded 2026-09-20:** this section previously described the pattern below as an unconfirmed perf-risk flag, not proven to cause the reported CODA-2533 slowness. It has since been **confirmed as the actual root cause** of the Events Landing Page infinite-spinner bug, and a fix has been applied to the specific view involved. §13 above (the client-side fetch timeout) remains valid as a separate, complementary defense-in-depth fix, but it was not what broke the page in production.

**What was wrong:** `webengine/views/events` (the `/events.json` endpoint) had, in all three model branches — Territories (`6-deab97cfd9-wb5km4`), Divisions (`6-acb19a94bd-4q8ftj`), Locations (`6-b4c9aba69c-h2nqvm`) — a query of the shape:
```
{{each events as event WHERE find_in_set('{$zuid}', territories) }}
```
with **no `LIMIT` and no `SORT`** — an unbounded scan of the *entire* shared `events` content model on every request, before the per-row `find_in_set` filter is even applied. This is the only `find_in_set`-over-a-large-model view in the repo that skipped the `LIMIT` convention every comparable view follows (e.g. `webengine/views/find-event.json` uses `LIMIT 1`, `webengine/views/get-stories-by-location.json` uses `limit 0, 10000`, `webengine/views/datasets/mobile_editor/content_list/events.json` uses `LIMIT {$skip}, {$limit}`).

**Confirmed live:** this unbounded scan was hitting WebEngine's hard **3000 query-execution cap**. Reproduced by hitting `/events.json?zuid=7-b4aeafb8dd-033tjr&model=6-deab97cfd9-wb5km4` (USA Southern Territory) on the branch preview and getting a "WebEngine Query Error — Maximum query limit of 3000 reached. Query execution halted." page. Critically, **this error page is a stage/preview-only diagnostic and is suppressed on production/live renders** — so in production the front-end just never received valid JSON, which is why the page hung on an infinite spinner instead of showing any error. This is why the bug looked, from the outside, exactly like a hung client-side fetch (§13's original hypothesis) even though the real cause was server-side.

**Is it Southern-Territory-specific?** No. The scan cost is driven by the *total row count* of the shared `events` content model (scanned before the per-row filter is applied), not by how many events happen to be tagged to a specific territory/division/location. Southern likely just surfaced it first because of load or timing — every territory/division/location is exposed to the same risk as the shared `events` model grows.

**Fix:** added `LIMIT 0, 500 SORT BY event.event_date_with_time DESC` to all three model branches in `webengine/views/events`. Sorting newest-first means that if truncation ever happens, oldest events are dropped first, not an arbitrary slice.

**Residual risks (flagged, not resolved by this fix):**
1. It's assumed — inferred from other `LIMIT`-bounded patterns elsewhere in the repo working correctly, not independently verified against a live API call — that WebEngine's query engine short-circuits the scan once `LIMIT` rows are matched, rather than scanning the full model and truncating output afterward. Worth having `api-integrator` confirm if the query-cap error recurs after this ships.
...
3. Recurring events that only populate `date_time_summary` (no `event_date_with_time`) have no comparable field for the new `SORT BY` and could sort toward the bottom, risking truncation first in a high-volume territory — not fixable without a schema/content change.
4. The earlier client-side `AbortController` timeout fix in `modules/upcoming-events` (§13) is **unchanged** and still in place as defense-in-depth for genuine network hangs — it's a separate, complementary fix, not superseded by this query fix.

**This is not isolated to `events.json`** — a repo-wide search turns up the same unbounded `find_in_set(...)` inside a `WHERE` clause in ~30+ other views (a mix of Block Library blocks, page modules, and ajax-json endpoints, e.g. `modules/programs-schedule`, `modules/services-service-area`, `modules/contact-us-officers`, `stories.json`, `staff.json`, several `datasets/mobile_editor/**` views). None of these are confirmed to have hit the query cap yet — `webengine/views/events` is the only one verified live so far — but given that this exact pattern just caused a real production bug, it's worth treating as a systemic question (e.g. "should relationship filtering use an indexed join, or at minimum always carry a `LIMIT`, instead of an unbounded `find_in_set` scan") rather than assuming the other ~30 are safe by default.
...
7. **CODA-2533 fixes** (§13 and §14) are Draft — pending review, implemented on branch `coda-2533` only, not yet QA'd/merged/deployed. Do not cite either as shipped until a human confirms the merge. §14's `LIMIT`/`SORT` fix in `webengine/views/events` is the confirmed fix for the reported bug; §13's fetch-timeout in `modules/upcoming-events` is a separate, still-valid defense-in-depth fix for genuine network hangs, kept unchanged. Other client-side `fetch()` calls in this repo (`stats.json`, `staff.json`, `stories.json`, `get-services-*.json`, etc.) still don't have an equivalent timeout guard — worth a proactive audit independent of CODA-2533.
8. **`find_in_set()` scan pattern recurring in ~30+ views repo-wide** (§14) — one instance (`webengine/views/events`) is now confirmed to have hit WebEngine's 3000-query cap in production and has been fixed with `LIMIT`/`SORT`. The other ~30 are not yet confirmed to have hit the same cap, but given a real instance of this exact pattern just caused a production bug, worth a proactive `web-developer` audit (add `LIMIT` to any that lack one) rather than waiting for each to fail individually.
9. **Unconfirmed assumption re: WebEngine `LIMIT` short-circuit behavior** (§14, residual risk #1) — worth an `api-integrator` live-API check if the "Maximum query limit of 3000 reached" error recurs after the CODA-2533 fix ships, to confirm `LIMIT` actually bounds scan cost rather than just output size.
