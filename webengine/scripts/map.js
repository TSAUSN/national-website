...
// Lazy-load Google Maps only when a map enters the viewport
...
function setupLazyMapLoading() {
  const mapTargets = document.querySelectorAll('.goggle-map, .event__map');
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', setupLazyMapLoading, { once: true });
} else {
  setupLazyMapLoading();
}
...
function handleMarkerContent(markerData) {
  ...
  const html = `
    ...
    `;
  const getServiceTypeForMap = async function(serviceTypeZuid) {
    try {
  ...
  fetch(`${window.location.origin}/services.json?location=${markerData.zuid}`)
    .then(res => res.json())
    .then(services => renderServicesForMap(Array.isArray(services) ? services : []))
    .catch((error) => {
      console.error('Error loading services:', error);
      const servicesContainer = document.getElementById('services-container');
      if (servicesContainer) servicesContainer.innerHTML = 'Error loading services';
    });
            ...
            if (status === google.maps.places.PlacesServiceStatus.OK && predictions) {
              ...
              `;
                ...
                item.addEventListener('click', function () {
                  ...
                  inputMobile.setAttribute(
                    'data-location-search-term-type',
